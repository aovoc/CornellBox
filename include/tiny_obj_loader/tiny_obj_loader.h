...
void LoadMtl(std::map<std::string, int> &material_map,
  ...
  while (inStream.peek() != -1) {
	...
	if ((0 == strncmp(token, "color", 5)) && IS_SPACE(token[5])) {
		token += 6;
		float r, g, b;
		parseFloat3(r, g, b, token);
		material.color[0] = r;
	...
	if ((0 == strncmp(token, "alpha", 5)) && IS_SPACE(token[5])) {
		token += 6;
		float alpha;
		alpha = parseFloat(token);
		material.alpha = alpha;
		continue;
	}

	if ((0 == strncmp(token, "mtl_type", 8)) && IS_SPACE(token[8])) {
		token += 9;
		std::string mtl_name = token;
		if (mtl_name == "DIFF") material.type = DIFF;
		else if (mtl_name == "SPEC") material.type = SPEC;
		else if (mtl_name == "REFR") material.type = REFR;
		else if (mtl_name == " ROUGHPLASTIC") material.type =  ROUGHPLASTIC;
		else{
			printf("error material type!");
		}
		continue;
	}
